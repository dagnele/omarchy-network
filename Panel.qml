...
Panel {
  id: root
  moduleName: "dagnele.network"
  ipcTarget: "dagnele.network"
  ...
  onBusyChanged: if (busy) passwordRevealed = false
  ...
  readonly property bool canJoinViaQr: canToggleWifi && !connectedWifiNetwork
  ...
  IpcHandler {
    ...
    function showJoinQr() { root.open(); root.joinViaQr() }
  ...
  // Public shell IPC keeps these actions available without clone-only permissions.
  // The direct IPC route forces self-detection instead: details polling stops while the
  ...
  function summonSpeedTest() {
    ...
    Quickshell.execDetached(["omarchy-shell", "shell", "summon", "omarchy.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}"])
  }
  ...
  function joinViaQr() {
    if (busy || !canJoinViaQr || !Networking.wifiEnabled || qrMode) return
    cancelPasswordPrompt()
    qrMode = true
    qrJoin.start()
  }
  ...
  KeyboardPanel {
    ...
    contentHeight: panel.fittedContentHeight(root.qrMode ? qrJoin.implicitHeight : column.implicitHeight)
    ...
    PanelKeyCatcher {
      ...
      blocked: root.passwordSsid !== "" || root.qrMode
    ...
    Column {
      ...
      RowLayout {
        width: parent.width
        visible: root.wifiStationAvailable && (root.scanning || root.canJoinViaQr)
        spacing: Style.space(8)
        ...
        Button {
          visible: root.canJoinViaQr
          text: "Scan QR"
          iconText: "󰐲"
          tooltipText: Networking.wifiEnabled
            ? "Scan your phone’s Wi-Fi QR code with the camera (Q)"
            : "Turn Wi-Fi on to scan a QR code"
          foreground: root.bar.foreground
          fontFamily: root.bar.fontFamily
          fontSize: Style.font.bodySmall
          horizontalPadding: Style.space(6)
          verticalPadding: Style.space(2)
          enabled: !root.busy && Networking.wifiEnabled
          focusable: true
          Accessible.role: Accessible.Button
          Accessible.name: "Join Wi-Fi by scanning a QR code"
          onClicked: root.joinViaQr()
        }
      }
